...
from keyboards import get_screenshot_review_keyboard, get_deposit_done_keyboard
...
class OnboardingHandler:
    ...
    def register(self):
        ...
        def handle_reject(call: CallbackQuery):
            # Wrong ID → same flow as old Reject
            self._handle_admin_decision(call, accept=False)

        @bot.callback_query_handler(func=lambda call: call.data.startswith("ss_nodeposit_"))
        def handle_no_deposit(call: CallbackQuery):
            self._handle_no_deposit(call)

        @bot.callback_query_handler(func=lambda call: call.data.startswith("ss_deposit_done_"))
        def handle_deposit_done(call: CallbackQuery):
            self._handle_deposit_done(call)
    ...
    # ------------------------------------------------------------------
    # Step 6: 9-Digit Trading Account ID
    # ------------------------------------------------------------------
    ...
    def _notify_updates_channel_only(self, registration_id, registration_data, user):
        """Send verification card ONLY to Updates Channel."""
    ...
    # ------------------------------------------------------------------
    # Admin Approve / Wrong ID Decision
    # ------------------------------------------------------------------
    def _handle_admin_decision(self, call: CallbackQuery, accept: bool):
        ...
        try:
            ...
            if not registration or registration.get("verification_status") not in ["pending", "no_deposit"]:
                try:
            ...
            if accept:
                try:
                    self.bot.answer_callback_query(call.id, "Approving & generating VIP link...")
                except Exception:
                    pass
                ...
                # Wrong ID flow (same as old Reject)
                try:
                    self.bot.answer_callback_query(call.id, "Wrong ID - Rejecting...")
                except Exception:
                    pass
                ...
                database.update_registration(registration_id, {
                    ...
                    "rejection_reason": "Wrong ID / Registration declined by admin"
                })
    ...
    # No Deposit Flow
    # ------------------------------------------------------------------
    def _handle_no_deposit(self, call: CallbackQuery):
        try:
            if not config.is_admin(call.from_user.id):
                try:
                    self.bot.answer_callback_query(call.id, "Unauthorized")
            ...
            try:
                self.bot.answer_callback_query(call.id, "Marked as No Deposit")
            except Exception:
                pass

            # Mark registration as no_deposit (so it can be re-submitted later)
            database.update_registration(registration_id, {
            ...
            database.update_user(telegram_id, {
                ...
                "onboarding_state": STATE_PENDING_APPROVAL,  # keep so Done can work
                "hot_lead_active": False
            })

            # Send message to user with Done button
            no_deposit_msg = (
            ...
            try:
                ...
                self.bot.send_message(
                    telegram_id,
                    no_deposit_msg,
                    reply_markup=get_deposit_done_keyboard(registration_id)
                )
            except Exception as e:
                logger.error(f"Failed to send No Deposit message to {telegram_id}: {e}")
            ...
            logger.info(f"Marked registration #{registration_id} as No Deposit for user {telegram_id}")

        except Exception as e:
            logger.error(f"Failed to handle No Deposit: {e}")

    # ------------------------------------------------------------------
    # User clicked Done after depositing
    # ------------------------------------------------------------------
    def _handle_deposit_done(self, call: CallbackQuery):
        try:
            registration_id = int(call.data.split("_")[3])
            registration = database.get_registration(registration_id)

            if not registration:
                try:
                    self.bot.answer_callback_query(call.id, "Registration not found")
                except Exception:
                    pass
                return
            ...
            # Only the same user can click Done
            ...
            if registration.get("verification_status") != "no_deposit":
                try:
                    self.bot.answer_callback_query(call.id, "Already processed or not in No Deposit state")
                except Exception:
                    pass
                return

            try:
                self.bot.answer_callback_query(call.id, "Submitting again for approval...")
            except Exception:
                pass

            # Reset status back to pending (reuse same registration record)
            database.update_registration(registration_id, {
                "verification_status": "pending",
                "verified_by": None,
                "verified_at": None,
            ...
            database.update_user(telegram_id, {
                "verification_status": "pending",
                "registration_status": "pending_verification",
                "onboarding_state": STATE_PENDING_APPROVAL,
                "last_activity": get_current_timestamp()
            })

            # Prepare existing data for re-sending to approval channel
            reg_data = registration.get("registration_data") or {}
            ...
            # Re-send the same approval card
            self._notify_updates_channel_only(registration_id, reg_data, user)

            try:
                self.bot.edit_message_text(
                    call.message.text + "\n\n✅ Submitted again for approval. Please wait...",
                    call.message.chat.id,
                    call.message.message_id
                )
            except Exception:
            ...
            logger.info(f"User {telegram_id} clicked Done — registration #{registration_id} re-submitted for approval")

        except Exception as e:
            logger.error(f"Failed to handle deposit Done: {e}")
