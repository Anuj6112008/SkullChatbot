...
def _user_already_started_bot(user_data) -> bool:
    """
    Check if user has already interacted with the bot before.
    If yes → do NOT restart onboarding / follow-up flow (prevents overlapping).
    """
    if not user_data:
        return False

    # If onboarding_state is already set → user has started the flow
    state = user_data.get("onboarding_state")
    if state and str(state).strip() and str(state).lower() not in ["none", "null", ""]:
        return True

    # If registration / verification already happened
    reg_status = user_data.get("registration_status")
    ver_status = user_data.get("verification_status")

    if reg_status and str(reg_status).lower() not in ["none", "null", ""]:
        return True

    if ver_status and str(ver_status).lower() in [
        "approved", "pending", "rejected", "no_deposit", "pending_verification"
    ]:
        return True
...
def register_join_request_handlers(bot: TeleBot, onboarding_service: OnboardingService = None):
    ...
    def handle_chat_join_request(join_request: ChatJoinRequest):
        try:
            ...
            # Support BOTH free channels
            free_channel_ids = config.get_free_channel_ids()
            if str(chat_id) not in free_channel_ids:
                return

            logger.info(f"Join request received from user {telegram_id} for free channel {chat_id}")
            ...
            if result.get("success") and result.get("action") == "approved":
                # IMPORTANT: If user already started bot before → DO NOT send intro / follow-ups again
                if _user_already_started_bot(user_data) and not is_new_user:
                    logger.info(
                        f"User {telegram_id} already started bot earlier. "
                        f"Skipping onboarding intro after joining channel {chat_id}"
                    )
                    return
    ...
    def approve_join_command(message: Message):
        try:
            ...
            if not user:
                ...
                return
            try:
                # Approve on primary free channel (existing behaviour)
