...
def get_screenshot_review_keyboard(registration_id):
    """Admin approval buttons: Approve / Wrong ID / No Deposit"""
    keyboard = InlineKeyboardMarkup(row_width=1)
    btn_approve = InlineKeyboardButton("✅ Approve", callback_data=f"ss_accept_{registration_id}")
    btn_wrong_id = InlineKeyboardButton("❌ Wrong ID", callback_data=f"ss_reject_{registration_id}")
    btn_no_deposit = InlineKeyboardButton("🚫 No Deposit", callback_data=f"ss_nodeposit_{registration_id}")
    keyboard.add(btn_approve)
    keyboard.add(btn_wrong_id)
    keyboard.add(btn_no_deposit)
    return keyboard


def get_deposit_done_keyboard(registration_id):
    """Button shown to user after No Deposit decision"""
    keyboard = InlineKeyboardMarkup(row_width=1)
    btn_done = InlineKeyboardButton("Done ✅", callback_data=f"ss_deposit_done_{registration_id}")
