...
class Config:
    ...
    FREE_CHANNEL_ID_2 = os.getenv("FREE_CHANNEL_ID_2", "")  # Second free channel
    ...
    def validate(self):
        ...
        if self.FREE_CHANNEL_ID_2:
            try:
                int(self.FREE_CHANNEL_ID_2)
            except ValueError:
                sys.exit("FREE_CHANNEL_ID_2 must be a valid integer")
    ...
    def get_channel_ids(self):
        ...
        if self.FREE_CHANNEL_ID_2:
            channels.append(int(self.FREE_CHANNEL_ID_2))
        if self.PAID_CHANNEL_ID:
    ...
    def get_free_channel_ids(self):
        """Return list of all free channel IDs that should trigger the onboarding flow."""
        ids = []
        main_id = self.get_free_channel_id()
        if main_id:
            ids.append(str(main_id))
        if self.FREE_CHANNEL_ID_2:
            ids.append(str(self.FREE_CHANNEL_ID_2).strip())
        return ids
